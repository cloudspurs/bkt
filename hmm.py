...
class hmm:
    ...
    def baum_welch(self, os, os_length, delta, max_iteration=float('inf')):
        loop_number = 0
        ...
        while True:
            ...
            for i in range(self.h):
                ...
                for k in range(self.o):
                    ...
                    #self.ho[i][k] = numerator_B / denominator_B

            now_delta = log_prob - log_prob_prev
            log_prob_prev = log_prob
            loop_number += 1

            if (now_delta <= delta) or (loop_number >= max_iteration):
                break

        #return self.pi, self.hh, self.ho, loop_number, alpha
        return alpha, loop_number
